...
with torch.no_grad():
    ...
    text_inputs = clip.tokenize(all_input_sents).to(device)
        ...
        # text = clip.tokenize((sent, prop, bind, rel)).to(device)
    ...
    image_features = np.tile(image_features, (4, 1)) # repeat images to match the size of the text features (4 * n_sents) trials
    ...
    # similarity_property = np.diag(all_similarities[:,n_sents:n_sents*2]).tolist()
    # similarity_binding = np.diag(all_similarities[:,n_sents*2:n_sents*3]).tolist()
    # similarity_relation = np.diag(all_similarities[:,n_sents*3:n_sents*4]).tolist()
...
y = np.hstack((np.ones(n_sents), np.zeros(n_sents*3)))
