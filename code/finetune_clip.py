...
class HomemadeClip(nn.Module):
    """
    Finetuning CLIP while keeping some layers frozen
    """
    def __init__(self, model_name='RN50x16', device='cpu', trainable=True):
        super().__init__()
        model, preprocess = clip.load(args.model, device=device)
        self.model = model
        self.preprocess = preprocess
        self.classificationHead = nn.Linear(768*2, 2)
    ...
    def forward(self, image_inputs, text_inputs):
        ...
        logits = self.classificationHead(torch.hstack((image_features, text_features)))
...
out_fn = f"openai-{args.model.replace('/', '_')}_{args.image_input_dir.replace('/', '_')}-2obj"
...
image_inputs = torch.stack([model.preprocess(img) for img in all_imgs]).to(device)
image_inputs = torch.tile(image_inputs, (4,1,1,1))
Y = torch.stack((torch.cat((torch.ones(n_sents), torch.zeros(n_sents*3))),
                torch.cat((torch.zeros(n_sents), torch.ones(n_sents*3))))).T
sampling_weights = torch.cat((torch.ones(n_sents)*3, torch.ones(n_sents*3)))
n_trials = len(Y)
torch.autograd.set_detect_anomaly(True)
full_dataset = torch.utils.data.TensorDataset(image_inputs, text_inputs, Y)

# train_dataset, test_dataset = torch.utils.data.random_split(all_sentences, [n_sents-test_size, test_size], generator=torch.Generator().manual_seed(seed))
train_dataset, test_dataset = torch.utils.data.random_split(full_dataset, [n_trials-test_size, test_size], generator=torch.Generator().manual_seed(seed))
sampler = torch.utils.data.sampler.WeightedRandomSampler(sampling_weights, batch_size)
train_dataloader = torch.utils.data.DataLoader(train_dataset, sampler=sampler, batch_size=batch_size, shuffle=True)

## Run the model
start_time = time.time()
print(f"Starting finetuning CLIP on {n_sents}*4 sentences and {len(all_imgs)} images")
for epo in tqdm(range(n_epochs)):
    ...
    ## Test
    with torch.no_grad():
        img, txt, y = test_dataset.dataset.tensors
        probs = model(img, txt)
        set_trace()
        # preds = probs.argmax(0)
        accuracy = probs.argmax(0) == y.argmax(0)
...
criterion = 33.5
df["choice"] = df["similarity"].apply(lambda x: x > criterion)
df["Perf"] = df.apply(lambda x: 1 if (x['choice'] and x['Violation']=='No') or (not x['choice'] and x['Violation']=='Yes') else 0, axis=1)
